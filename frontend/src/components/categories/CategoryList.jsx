    ...
    <div className="max-w-screen-xl mx-auto px-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        {categories.map(cat => (
          <Link
            key={cat.id}
            to={`/category/${cat.slug}`} onClick={() => window.scrollTo(0, 0)}
            className="block p-6 bg-gray-100 rounded-lg hover:shadow-lg transition-shadow duration-200"
          >
            <h3 className="text-lg font-medium text-gray-800">{cat.name}</h3>
            <p className="text-sm text-gray-500 mt-1">View all articles</p>
          </Link>
        ))}
      </div>
    </div>
